...
<div class="bg">
   ...
   <aside class="main-sidebar">
    ...
    <ul class="bread">
      ...
      <li><a href="##" id="a_leader_txt">运输</a></li>
    </ul>
   ...
   <section class="list_data">
			<table id="dg" title="" class="easyui-datagrid" border="0" cellspacing="0" style="width:88%;text-align: center;"
					data-options="toolbar:'#toolbar',pagination:true,singleSelect:true,
                        url:'Transport_selectTransport.action',pageSize:10,pageList:[10,20,30]">
					<thead>
						<tr>
							<th field="tnum" align="center"    width="20%">订单号</th>
							<th field="tdate"  align="center"   width="20%">送货日期</th>
							<th field="tcompany"  align="center" width="20%">送货单位</th>
							<th field="tperson" align="center"  width="20%">送货人</th>
														<th field="tr" align="center"  width="11%">接收人</th>
							
							<!-- <th field="tother" align="center"  width="20%">备注</th> -->
	...
	<div class="ftitle" style="text-align: center;font-size: 14px;margin-bottom: 10px;">运输信息</div>
	<form id="fm" method="post">
		...
		<div class="fitem">
			<label>订单号</label>
			<input name="tnum" class="easyui-validatebox" required="true" style="margin-left:13px;">
		</div><br>
		<div class="fitem">
			<label>送货日期&nbsp;&nbsp;</label>
			<input name="tdate" class="easyui-datebox" required="true" style="margin-left:10px;width:170px;">
		</div><br>
		<div class="fitem">
			<label>送货单位</label>
			<input name="tcompany" class="easyui-validatebox" required="true" style="margin-left:5px;">
		</div><br>
		<div class="fitem">
			<label>送货人</label>
			<input name="tperson" class="easyui-validatebox" required="true" style="margin-left:13px;">
		</div><br>
		<div class="fitem">
			<label>接收人</label>
			<input name="tother" class="easyui-validatebox" required="true" style="margin-left:13px;">
		</div><br>		
	</form>
</div>
<div id="dlg-buttons">
...
//新用户
function newUser(){
	var  add= '<%= session.getAttribute("添加运输信息") %>';
	if(add=="null"||add==""){
		alert("没有权限")
	}else{
		$('#dlg').dialog('open').dialog('setTitle','添加运输信息');
		$('#fm').form('clear');	}
}
//更新
function editUser(){
var  add= '<%= session.getAttribute("编辑运输信息") %>';
	...
	if(add=="null"||add==""){
		...
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','编辑运输信息');
			$('#fm').form('load',row);
		}	
		}
}
...
function saveUser(){
	$('#fm').form('submit',{
		url: 'Transport_saveTransport.action',
		onSubmit: function(){
...
var  add= '<%= session.getAttribute("删除运输信息") %>';	
	if(add=="null"||add==""){
		alert("没有权限")
	}else{
		var row = $('#dg').datagrid('getSelected');
		if (row){
			$.messager.confirm('Confirm','是否删除运输信息？',function(r){
				if (r){
					$.post('Transport_deleteTransport.action',{id:row.id},function(result){
						/* if (result.success){
