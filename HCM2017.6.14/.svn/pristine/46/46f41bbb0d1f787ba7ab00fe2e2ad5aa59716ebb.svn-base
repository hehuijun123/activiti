...
<div class="bg">
  ...
  <aside class="main-sidebar">
    ...
    <ul class="bread">
      ...
      <li><a href="##" id="a_leader_txt">废弃物处置</a></li>
    </ul>
    ...
    <section class="list_data">
			<table id="dg" title="" class="easyui-datagrid" border="0" cellspacing="0" style="width:88%;text-align: center;"
					data-options="toolbar:'#toolbar',pagination:true,singleSelect:true,
                        url:'Waste_selectWaste.action',pageSize:10,pageList:[10,20,30]">
					<thead>
						<tr>
							<th field="wname" align="center"    width="20%">废弃物类型</th>
							<th field="wdate"    align="center" width="20%">时间</th>
							<th field="wnum" align="center"  width="20%">数量</th>
							<th field="wcompany"  align="center" width="20%">接收单位</th>
							<th field="wperson" align="center"  width="20%">处置人</th>
							<th field="wr" align="center"  width="11%">记录人</th>							
						</tr>
					</thead>
				</table>
				<div id="toolbar">
	...
	<div class="ftitle" style="text-align: center;font-size: 14px;margin-bottom: 10px;">废弃物信息</div>
	<form id="fm" method="post">
		...
		<div class="fitem">
			<label>废弃物类型&nbsp;&nbsp;&nbsp;&nbsp;</label>	
			<select name="wname" id="wname" class="easyui-combobox" required="true" style="margin-left:25px;width:175px"  > 
							   	 <option></option>  
									<option value="废酸">废酸</option>  
									<option value="废碱">废碱</option>  
									<option value="农药废物">农药废物</option>  
									<option value="废药物、药品试剂">废药物、药品试剂</option> 
													
			</select>  
		</div><br>
		<div class="fitem">
			<label>时间&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>
			<input name="wdate" class="easyui-datebox" required="true" style="margin-left:25px;width:175px;">
		</div><br>
		<div class="fitem">
			<label>数量</label>
			<input name="wnum" class="easyui-validatebox" required="true" style="margin-left:25px;">
		</div><br>
		<div class="fitem">
			<label>接收单位</label>
			<input name="wcompany" class="easyui-validatebox" required="true" style="margin-left:5px;">
		</div><br>
		<div class="fitem">
			<label>处置人</label>
			<input name="wperson" class="easyui-validatebox" required="true" style="margin-left:13px;">
		</div><br>
...
//新用户
function newUser(){
	var  add= '<%= session.getAttribute("添加废弃物信息") %>';
	if(add=="null"||add==""){
		alert("没有权限")
	}else{
		$('#dlg').dialog('open').dialog('setTitle','添加废弃物信息');
		$('#fm').form('clear');	}
}
//更新
function editUser(){
var  add= '<%= session.getAttribute("编辑弃物信息") %>';
	...
	if(add=="null"||add==""){
		...
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','编辑弃物信息');
			$('#fm').form('load',row);
		}	
		}
}
...
var  add= '<%= session.getAttribute("删除弃物信息") %>';
	...
	if(add=="null"||add==""){
		...
		var row = $('#dg').datagrid('getSelected');
		if (row){
			$.messager.confirm('Confirm','是否删除弃物信息？',function(r){
				if (r){
					$.post('Waste_deleteWaste.action',{id:row.id},function(result){
						/* if (result.success){
