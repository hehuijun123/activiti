...
public class JxlExcelCreate {

    /*public static void main(String[] args) throws Exception {
        writeExcel();
    }
*/
    public static void writeExcelSc(Map<String,List<Excel>> map,Manager manager) throws Exception {
        // 第一步创建excel文件,并决定excel的路径和文件名
    	
    	String name = "E:/excel/"+manager.getMfactory()+"-"+GetDate.Getdate()+".xls";
    	String Ename =manager.getMfactory()+"-"+GetDate.Getdate();//文件名
    	String sc = manager.getMfId();
    	String Edate = GetDate.Getdate();
    	save(Ename,sc,Edate,name);
 ...
 /*       sheet.mergeCells(0, 0, 7, 0); // 合并单元格A-h列共8列
        ...
        sheet.addCell(new Label(0, 1, "填报单位", contentFormat2));//列，行
        sheet.mergeCells(1, 1, 3, 1); // 合并单元格B-G列共7列
        sheet.addCell(new Label(1, 1, manager.getMfactory(), contentFormat2));
        ...
        sheet.mergeCells(1, 1, 2, 1); // 合并单元格B-G列共7列
        sheet.addCell(new Label(1, 1, manager.getMfactory(), contentFormat2));
        sheet.addCell(new Label(3, 1, "填报人", contentFormat2));
        sheet.addCell(new Label(4, 1, manager.getMname(), contentFormat2));
        sheet.mergeCells(6, 1, 7, 1); // 合并单元格B-G列共7列
        sheet.addCell(new Label(5, 1, "填报日期", contentFormat2));
        sheet.addCell(new Label(6, 1, date, contentFormat2));
        sheet.mergeCells(0, 2, 7, 2); // 合并单元格A-h列共8列
        sheet.addCell(new Label(0, 2, "一、危险化学品余量", contentFormat2));
        ...
        // 这里的数据可以从数据库里获取,个人建议将这些抽成共通的方法,提供导入数据,用时调用即可
        	 ...
        	 int m = 1;
        	 Excel sh = null;
        	 for(Map.Entry<String, List<Excel>> entry : map.entrySet()){
        		 List<Excel> list = entry.getValue();
        		 a += list.size();
...
*/                   sheet.addCell(new Label(1, j, sh.getEcode(), contentFormat));
                     sheet.addCell(new Label(2, j, sh.getEtype(), contentFormat));
                     sheet.addCell(new Label(3, j, sh.getEname(), contentFormat));
                     sheet.addCell(new Label(4, j, sh.getEbsurplus(), contentFormat));
                     sheet.addCell(new Label(5, j, sh.getEpurchase(), contentFormat));
                     sheet.addCell(new Label(6, j, sh.getEtsurplus(), contentFormat));
                     sheet.addCell(new Label(7, j, sh.getEother(), contentFormat));
                     i++;
                     if(i>=list.size()){
                    	 i = 0;
                     }
    ...
    public static void writeExcelJT(Map<String,List<Excel>> map) throws Exception {
        // 第一步创建excel文件,并决定excel的路径和文件名
    	
    	String name = "E:/excel/城投水务（集团）有限公司-"+GetDate.Getdate()+".xls";
