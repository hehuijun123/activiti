...
$(function(){
	...
	$('#dg').datagrid({
		url:'HC_selectHazardousChemicals.action',
		pagination:true,
		singleSelect:true,
		pageSize:20,
		pageList:[10,20,30],
		columns:[[
			{title:'类型',field:"htype",width:'16%',align:'center'},
			{title:'品名',field:"name",width:'16%',align:'center',formatter:function(value,row,index){
				console.log(row.msds)
				var href="http://localhost:8080/HCM/msds/"+row.msds;
				var lookup = "" ;					  //onclick="window.open('http://www.somsds.com/whp.htm','_blank');void 0"
				lookup = '<a href="'+href+'" target="_blank" class="easyui-linkbutton l-btn" plain="true">'+row.name+'</a>';	
				console.log(lookup)
				return lookup;									
				
			}},
			{title:'2015编码',field:"hnum",width:'16%',align:'center'},
			{title:'别名',field:"othername",width:'16%',align:'center'},
			{title:'CAS号',field:"cASNum",width:'16%',align:'center'},
			{title:'使用场合',field:"hmethod",width:'16%',align:'center'},
			 /* {title:'使用场合',field:"#",width:'20%',align:'center', formatter:function(value,row,index){
				var lookup = "" ;					
			...
			for(var i = 0;i<data.rows.length;i++){
				...
				htype[i] = HazardousChemicals.htype;
			}
			for(var i = 0;i<data.rows.length;i++){
				var HazardousChemicals = data.rows[i];
				htypep[i] = HazardousChemicals.htype;
			}
			$.unique(htype);//去重复
			 for(var i = 0;i<htype.length;i++){
				...
				for(var j = 0;j<data.rows.length;j++){
					...
					htypes = HazardousChemicals.htype;
					if(htype[i]==htypes){		
						num +=1;
					}
					indexs =j; 
...
<div class="bg">
   <div class="toparticle">
    ...
    <ul class="bread">
      ...
      <li><a href="##" id="a_leader_txt">危化品MSDS</a></li>
    </ul>
     ...
     <section class="list_data">
			<!-- <table id="dg" title="" class="easyui-datagrid" border="0" cellspacing="0" style="width:88%;text-align: center;"
					data-options="toolbar:'#toolbar',pagination:true,singleSelect:true,
                        url:'HC_selectHazardousChemicals.action',pageSize:10,pageList:[10,20,30]">
					<thead>
						<tr>
						<th field="htype" align="center"  width="20%">类型</th>					
						<th field="name" align="center"    width="20%">品名</th>
						<th field="hnum"  align="center"  width="20%">2015编码</th>
						<th field="othername" align="center"  width="20%">别名</th>
						<th field="cASNum" align="center"  width="20%">CAS号</th>
						<th field="hmethod" align="center"  width="20%">使用场合</th>							
						</tr>
					</thead>
				</table> -->
				<div id="toolbar">
<!-- 	<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="newUser()">添加</a>
	...
	<a href="http://www.somsds.com/whp.htm"  target="_blank" class="easyui-linkbutton" iconCls="icon-anchor" plain="true" >&nbsp;&nbsp; 危化品目录</a>
  ...
  <div style="display: none;">   
	...
	<div class="ftitle" style="text-align: center;font-size: 14px;margin-bottom: 10px;">危化品信息</div>
	<form id="fm" method="post">
		...
		<div class="fitem">
			<label>品名</label>
			<input name="name" class="easyui-validatebox" required="true" style="margin-left:10px;">
		</div><br>
		<div class="fitem">
			<label>别名</label>
			<input name="othername" class="easyui-validatebox" required="true" style="margin-left:10px;">
		</div><br>
		<div class="fitem">
			<label>编码</label>
			<input name="hnum" class="easyui-validatebox" required="true" style="margin-left:10px;">
		</div><br>
		<div class="fitem">
			<label>CAS号</label>
			<input name="cASNum" class="easyui-validatebox" required="true" style="margin-left:5px;">
		</div><br>
		<div class="fitem">
			<label>备注</label>
			<input name="other" class="easyui-validatebox" required="true" style="margin-left:10px;">
		</div><br>
...
//新用户
function newUser(){
	var  add= '<%= session.getAttribute("添加危化品信息") %>';
	if(add=="null"||add==""){
		alert("没有权限")
	}else{
		$('#dlg').dialog('open').dialog('setTitle','添加危化品信息');
		$('#fm').form('clear');	}
}
//更新
function editUser(){
var  add= '<%= session.getAttribute("编辑危化品信息") %>';
	...
	if(add=="null"||add==""){
		...
		if (row){
			$('#dlg').dialog('open').dialog('setTitle','编辑危化品信息');
			$('#fm').form('load',row);
		}	
		}
}
...
function saveUser(){
	$('#fm').form('submit',{
		url: "HC_saveHazardousChemicals.action",
		onSubmit: function(){
...
var  add= '<%= session.getAttribute("删危化品信息") %>';
	...
	if(add=="null"||add==""){
		...
		var row = $('#dg').datagrid('getSelected');
		if (row){
			$.messager.confirm('Confirm','是否删除危化品？',function(r){
				if (r){
					$.post('HC_deleteHazardousChemicals.action',{id:row.id},function(result){
						/* if (result.success){
